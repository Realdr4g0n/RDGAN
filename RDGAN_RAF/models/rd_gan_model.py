...
class RDGANModel(BaseModel):
    """
    This class implements the rdgan model, for learning image-to-image translation without paired data.
    ...
    rdgan paper: https://arxiv.org/pdf/1703.10593.pdf
    """
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        For rdgan, in addition to GAN losses, we introduce lambda_A, lambda_B, and lambda_identity for the following losses.
        ...
        Dropout is not used in the original rdgan paper.
        """
        parser.set_defaults(no_dropout=True)  # default rdgan did not use dropout
        if is_train:
            parser.add_argument('--lambda_A', type=float, default=15.0, help='weight for cycle loss (A -> B -> A)')
            ...
            parser.add_argument('--lambda_expr', type=float, default = 10.0, help='use expr VGG19 ')
            parser.add_argument('--target', type=str, default ='disgusted', help='target domain eg) disgusted,fearful,angry')

        return parser

    def __init__(self, opt):
        """Initialize the rdgan class.
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'idt_A','expr_A','percept_A', 'D_B', 'G_B', 'cycle_B', 'idt_B','expr_B','percept_B']
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netVGG.load_state_dict(torch.load('./raf_train.t7'))
            self.netVGG.cuda()
        ...
        if self.isTrain:
            ...
            #self.criterionExpr = torch.nn.L1Loss()
            #self.criterionExpr = networks.GANLoss('raf').to(self.device)
...
################################################################################# 19.07.05 Expression loss 수정 / 
        ...
        self.loss_expr_A = self.criterionExpr(self.fake_Bscore ,torch.cuda.FloatTensor([0,0,1,0,0,0,0])) * lambda_expr#19.07.24 Expression loss 수정
